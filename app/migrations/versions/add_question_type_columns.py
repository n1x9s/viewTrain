...
def upgrade() -> None:
    ...
    op.execute("ALTER TABLE interviews ADD COLUMN IF NOT EXISTS question_type VARCHAR DEFAULT 'pythonn' NOT NULL")
    
    # Проверяем наличие колонки question_type в таблице user_answers и добавляем ее, если она не существует
    op.execute("ALTER TABLE user_answers ADD COLUMN IF NOT EXISTS question_type VARCHAR DEFAULT 'pythonn' NOT NULL")
