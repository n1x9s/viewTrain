...
class BaseDAO(Generic[T]):
    ...
    async def find_one_or_none(
        cls, session: AsyncSession, filters: Optional[Any] = None, **filter_by
    ) -> Optional[T]:
        """Поиск одной записи"""
        ...
        if filters is not None:
            ...
            for key, value in filter_dict.items():
                if value is not None:
                    stmt = stmt.filter(getattr(cls.model, key) == value)
    ...
    async def find_by_ids(cls, session: AsyncSession, ids: List[int]) -> List[T]:
        ...
        if not ids:
            return []
        stmt = select(cls.model).filter(cls.model.id.in_(ids))
        result = await session.execute(stmt)
