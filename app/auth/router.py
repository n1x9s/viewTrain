...
async def register_user(
    ...
    try:
        ...
        if existing_user:
            logger.info(
                f"User with email {user_data.email} already exists, attempting authentication"
            )
            # Проверяем пароль
        ...
        new_user = User(
            email=user_data.email, name=user_data.name, phone=user_data.phone
        )
...
async def get_me(
    ...
    stmt = (
        ...
        .options(selectinload(User.directions), selectinload(User.languages))
    )
...
async def update_me(
    ...
    try:
        ...
        directions = await DirectionsDAO.find_by_ids(
            session=session, ids=user_data.direction_ids
        )
        ...
        languages = await LanguagesDAO.find_by_ids(
            session=session, ids=user_data.language_ids
        )
...
async def delete_account(
    ...
    try:
        ...
        if interview_ids:
            ...
            await session.execute(delete(Interview).where(Interview.user_id == user.id))
...
async def select_directions(
    ...
    try:
        ...
        directions = await DirectionsDAO.find_by_ids(
            session=session, ids=direction_data.direction_ids
        )
        ...
        return DirectionSelectionResponse(
            message="Направления успешно обновлены", selected_directions=directions
        )
    except Exception as e:
        await session.rollback()
        raise HTTPException(
...
async def select_languages(
    ...
    try:
        ...
        languages = await LanguagesDAO.find_by_ids(
            session=session, ids=language_data.language_ids
        )
...
async def check_email(
    ...
    existing_user = await UsersDAO.find_one_or_none(
        session=session, filters=EmailModel(email=email_data.email)
    )
