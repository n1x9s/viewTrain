...
class Settings(BaseSettings):
    ...
    secret_key: str = Field(
        ..., alias="SECRET_KEY", description="Секретный ключ для JWT токенов"
    )
    algorithm: str = Field(
        "HS256", alias="ALGORITHM", description="Алгоритм для JWT токенов"
    )
    ...
    log_file: str = Field("app.log", alias="LOG_FILE", description="Путь к файлу логов")

    # GigaChat API
    gigachat_client_id: str = Field(
        ..., alias="GIGACHAT_CLIENT_ID", description="Client ID для GigaChat API"
    )
